...
def replace_unknowns(data):
    data = np.where(data == 'unknown',"",data)
    max_val = np.amax(data)
    data = np.where(data == "",max_val,data)
    return data
...
df_train = pd.read_csv('datasets/bank/train.csv').iloc[:5000,:]
df_test = pd.read_csv('datasets/bank/test.csv').iloc[:5000,:]
...
df_train[columns[3]] = replace_unknowns(df_train[columns[3]])
df_train[columns[8]] = replace_unknowns(df_train[columns[8]])
df_train[columns[15]] = replace_unknowns(df_train[columns[15]])
df_test[columns[3]] = replace_unknowns(df_test[columns[3]])
df_test[columns[8]] = replace_unknowns(df_test[columns[8]])
df_test[columns[15]] = replace_unknowns(df_test[columns[15]])
...
#split_criteria = int(input("Split Criteria: 1. Information gain, 2. Gini Index, 3. Majority Error? "))
